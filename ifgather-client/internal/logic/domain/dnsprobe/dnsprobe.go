...
func Run(subdomain []string) ([]*ResSubdomain, error) {
	...
	res := cdnCheck(resultMap)
	return res, nil
}

// CDN检测
func cdnCheck(r []*ResSubdomain) []*ResSubdomain {
	...
	for _, v := range r {
		...
		if cdnCheckIp(v.IP[0]) { // 优先通过ip段判断cdn
			v.Cdn = true
			result = append(result, v)
			continue
		}
		if len(v.CNAME) != 0 { // cname判断cdn
			if cdnCnameCheck(v.CNAME) {
				v.Cdn = true
				result = append(result, v)
				continue
			}
...
// 根据ip段检测cdn
func cdnCheckIp(ip string) bool {
...
func ParseCIDR() {
	...
	for _, c := range ips {
		...
		if err != nil {
			logger.LogDomain.Warningf(context.Background(), "添加CDN段失败:%s", err.Error())
			continue
		}
...
// 根据cname检测cdn
func cdnCnameCheck(cname []string) bool {
	for _, v := range cname {
		for k := range GCname {
