...
func (s *Scanner) TCPReadWorker() {
	...
	for {
		...
		if !strings.Contains(public.GOptions.Hosts, addr.String()) {
			if s.debug {
				logger.LogPortScan.Debugf(context.Background(), "[-] 丢弃来自非目标IP的TCP数据包 %s", addr.String())
			}
			continue
		}
		...
		if tcpLayer := packet.Layer(layers.LayerTypeTCP); tcpLayer != nil {
			...
			if tcp.DstPort != layers.TCPPort(s.listenPort) {
				if s.debug {
					logger.LogPortScan.Debugf(context.Background(), "[-] 丢弃来自 %s 的TCP数据包 %s:%d 端口不匹配", addr.String(), tcp.DstPort, s.listenPort)
				}
			} else if tcp.SYN && tcp.ACK {
				if s.debug {
					logger.LogPortScan.Debugf(context.Background(), "[-] 接受来自 %s:%d 的SYN+ACK数据包", addr.String(), tcp.DstPort)
				}
...
func (s *Scanner) SendAsyncPkg(ip string, port int, pkgFlag PkgFlag) {
	...
	if err != nil {
		if s.debug {
			logger.LogPortScan.Debugf(context.Background(), "[-] %s:%d 设置网络层失败: %s", ip, port, err)
		}
	} else {
		err = s.send(ip, s.tcpPacketlistener, &tcp)
		if err != nil {
			if s.debug {
				logger.LogPortScan.Debugf(context.Background(), "[-] 发送到 %s:%d 数据包失败:%s", ip, port, err)
			}
		}
	}
}
...
func (s *Scanner) TCPResultWorker() {
	for ip := range s.tcpChan {
		if s.State == Scan {
			...
			if s.debug {
				logger.LogPortScan.Debugf(context.Background(), "[+] 接收到 %s:%d 的回应", ip.ip, ip.port)
			}
