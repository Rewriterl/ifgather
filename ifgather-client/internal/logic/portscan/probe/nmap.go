...
func (N *NmapProbe) Init() {
	statikFS, err := fs.New()
	if err != nil {
		logger.LogPortScan.Fatalf(context.Background(), "[-] 静态资源FS初始化失败:%s", err.Error())
	}
	resources, err := statikFS.Open("/nmap.txt")
	if err != nil {
		logger.LogPortScan.Fatalf(context.Background(), "[-] 加载Nmap协议失败:%s", err.Error())
	}
	defer resources.Close()

	nmapContents, err := io.ReadAll(resources)
	if err != nil {
		logger.LogPortScan.Fatalf(context.Background(), "[-] 加载Nmap协议失败:%s", err.Error())
	}
	...
	if err != nil {
		logger.LogPortScan.Fatalf(context.Background(), "[-] 加载端口数据失败:%s\n", err.Error())
	}
	defer resources1.Close()
	nmapContentsPort, err := io.ReadAll(resources1)
	if err != nil {
		logger.LogPortScan.Fatalf(context.Background(), "[-] 加载端口数据失败:%s\n", err.Error())
	}
...
func (N *NmapProbe) parseProbesFromContent(content *string) {
	...
	if len(lines) == 0 {
		logger.LogPortScan.Fatalf(context.Background(), "[-] [端口扫描] nmap指纹库数据为空\n")
	}
	...
	for _, line := range lines {
		...
		if c > 1 {
			logger.LogPortScan.Fatalf(context.Background(), "[-] [端口扫描] nmap指纹库格式错误，只能有一个Exclude标识符，且该标识符应该在首行")
		}
	}
	...
	if !(strings.HasPrefix(l, "Exclude ") || strings.HasPrefix(l, "Probe ")) {
		logger.LogPortScan.Fatalf(context.Background(), "[-] [端口扫描] nmap指纹库解析失败，首行应该由Probe或Exclude标识符开始")
	}
...
func (p *Probe) parseProbeInfo(probeStr string) {
	...
	if !(proto == "TCP " || proto == "UDP ") {
		logger.LogPortScan.Fatalf(context.Background(), "[-] [端口扫描] 解析nmap指纹库失败，protocol字段必须为TCP或UDP")
	}
	if len(other) == 0 {
		logger.LogPortScan.Fatalf(context.Background(), "[-] [端口扫描] 解析nmap指纹库失败，探测名称描述字段名为空")
	}
	...
	if len(dataList) > 0 {
		...
		if err != nil {
			logger.LogPortScan.Fatalf(context.Background(), "[-] [端口扫描] nmap指纹库编码发送包失败[%s]:  %s\n", dataList[0], err)
		} else {
			p.Data = data_byte
		}
	}
...
func (N *NmapProbe) ScanWithProbe(host, port string, SocketTimeout int, resultChan chan *Task) {
	...
	if len(defaultProbe) > 0 {
		...
		for resp := range chanTask {
			logger.LogPortScan.Debugf(context.Background(), "[+] 默认端口指纹获取成功:%s:%s %s", host, port, resp.ServiceNmae)
	...
	if len(oneProbe) > 0 {
		...
		close(chanTask)
		for resp := range chanTask {
			logger.LogPortScan.Debugf(context.Background(), "[+] 级别1指纹获取成功:%s:%s %s", host, port, resp.ServiceNmae)
			N.HttpCheck(resp, SocketTimeout)
			resultChan <- resp
			return
		}
	...
	if len(sixProbe) > 0 {
		...
		close(chanTask)
		for resp := range chanTask {
			logger.LogPortScan.Debugf(context.Background(), "[+] 级别<6指纹获取成功:%s:%s %s", host, port, resp.ServiceNmae)
			N.HttpCheck(resp, SocketTimeout)
			resultChan <- resp
			return
		}
	...
	if len(nineProbe) > 0 {
		...
		close(chanTask)
		for resp := range chanTask {
			logger.LogPortScan.Debugf(context.Background(), "[+] 级别<9指纹获取成功:%s:%s %s", host, port, resp.ServiceNmae)
			N.HttpCheck(resp, SocketTimeout)
			resultChan <- resp
			return
		}
	...
	if ok {
		logger.LogPortScan.Debugf(context.Background(), "[-] 指纹探测失败:%s:%s %s", host, port, ServiceNmae)
		tasktmp := Task{}
		tasktmp.Addr = GetAddress(host, port)
		...
		logger.LogPortScan.Debugf(context.Background(), "[-] 未知服务:%s:%s", host, port)
		tasktmp := Task{}
		tasktmp.Addr = GetAddress(host, port)
		resultChan <- &tasktmp
		return
...
func (N *NmapProbe) taskSocket(address string, Indexes, SocketTimeout int, taskChan chan *Task) {
	...
	ok, extras := N.regxRespone(responeData, N.Probes[Indexes].Matches, N.Probes[Indexes].Fallback)
	if !ok { // 指纹识别失败
		return
	}
	taskChan <- &Task{
